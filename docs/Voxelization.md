...
3. **Palette:** only surface samples vote. Colors are grouped by hue (plus neutrals). Each group gets slots in proportion to how much of the surface it covers, then a gap-split median cut builds a **ramp** for that group. Voxels are remapped with HSB distance so a dusty sample snaps to its hue ramp instead of a nearby brown. Default target is 256 (`voxformat_targetcolors` `0`). One slot is reserved for exact white `(255, 255, 255)` and interiors use that index (surface remap runs first so pale petals do not snap to it). Interior voxels do not enter the histogram. Palette bytes are sRGB. glTF `baseColorFactor` is applied in linear space (skipped when the factor is 1) and encoded with the sRGB OETF so midtones are not crushed.
