# Reverse-domain app id matching SDL_SetAppMetadata in App.cpp (Wayland app_id / X11 WM_CLASS)
function(engine_app_id out_var target_name)
	set(${out_var} "com.github.${CMAKE_PROJECT_NAME}.${target_name}" PARENT_SCOPE)
endfunction()
...
function(engine_install TARGET FILE DESTINATION INSTALL_DATA)
	...
	if (APPLE)
		set_source_files_properties(${FILEPATH} PROPERTIES MACOSX_PACKAGE_LOCATION Resources/${DESTINATION})
	elseif (INSTALL_DATA)
		if (WIN32)
		...
		install(FILES ${FILEPATH} DESTINATION ${INSTALL_DATA_DIR}/${DESTINATION} COMPONENT ${TARGET})
	endif()
	configure_file(${FILEPATH} ${CMAKE_BINARY_DIR}/${PROJECT_NAME}/${DESTINATION}/${filename} COPYONLY)
endfunction()
...
function(engine_add_executable)
	...
	engine_app_id(APP_ID ${_EXE_TARGET})
	if (APPLE)
	...
	elseif(WIN32)
		...
		# Per-target names: several exes share one CMakeLists and would otherwise
		# overwrite a single application.rc (last target wins, often no icon).
		configure_file(${ROOT_DIR}/contrib/installer/windows/application.manifest.in ${CMAKE_CURRENT_BINARY_DIR}/${_EXE_TARGET}.manifest @ONLY)
		configure_file(${ROOT_DIR}/contrib/installer/windows/application.rc.in ${CMAKE_CURRENT_BINARY_DIR}/${_EXE_TARGET}.rc @ONLY)
		target_sources(${_EXE_TARGET} PRIVATE ${CMAKE_CURRENT_BINARY_DIR}/${_EXE_TARGET}.manifest)
		target_sources(${_EXE_TARGET} PRIVATE ${CMAKE_CURRENT_BINARY_DIR}/${_EXE_TARGET}.rc)
	elseif (EMSCRIPTEN)
		if (EXISTS ${CMAKE_CURRENT_SOURCE_DIR}/${_EXE_TARGET}.html)
			#em_link_js_library(${_EXE_TARGET} ${ROOT_DIR}/contrib/installer/emscripten/library.js)
			configure_file(${CMAKE_CURRENT_SOURCE_DIR}/${_EXE_TARGET}.html ${CMAKE_CURRENT_BINARY_DIR}/${_EXE_TARGET}.shell.html)
			set_target_properties(${_EXE_TARGET} PROPERTIES SUFFIX ".html")
			set_target_properties(${_EXE_TARGET} PROPERTIES LINK_FLAGS "--shell-file ${CMAKE_CURRENT_BINARY_DIR}/${_EXE_TARGET}.shell.html")
	...
	elseif(UNIX)
		if (_EXE_WINDOWED)
			set(DESKTOP_FILENAME "${APP_ID}.desktop")
			if (EXISTS ${ROOT_DIR}/contrib/installer/linux/${_EXE_TARGET}.desktop.in)
				configure_file(${ROOT_DIR}/contrib/installer/linux/${_EXE_TARGET}.desktop.in ${PROJECT_BINARY_DIR}/${DESKTOP_FILENAME})
			else()
				configure_file(${ROOT_DIR}/contrib/installer/linux/desktop.in ${PROJECT_BINARY_DIR}/${DESKTOP_FILENAME})
			endif()
			if (DESKTOP_FILE_VALIDATE_EXECUTABLE)
				add_custom_command(TARGET ${_EXE_TARGET} POST_BUILD
					COMMAND ${DESKTOP_FILE_VALIDATE_EXECUTABLE} ${PROJECT_BINARY_DIR}/${DESKTOP_FILENAME}
					COMMENT "Validate ${DESKTOP_FILENAME}"
				)
			endif()
			if (INSTALL_DATA)
				install(FILES ${PROJECT_BINARY_DIR}/${DESKTOP_FILENAME} DESTINATION ${INSTALL_APPLICATION_DIR} COMPONENT ${PROJECT_NAME})
		...
		if (MANPAGEPATH)
			...
			if (MAN_PAGE_HEADER)
				...
				message(WARNING "No .TH header found in ${MANPAGEPATH}, using man section ${MAN_PAGE_SECTION}")
			endif()
	...
	# PNG copy for SDL_SetWindowIcon fallback (platform icon may be .ico/.icns)
	set(ICON_PNG_PATH ${DATA_DIR}/${_EXE_TARGET}/${APPICON}.png)
	if (EXISTS ${ICON_PNG_PATH})
		configure_file(${ICON_PNG_PATH} ${CMAKE_BINARY_DIR}/${_EXE_TARGET}/${APPICON}.png COPYONLY)
		if (APPLE)
			set_source_files_properties(${ICON_PNG_PATH} PROPERTIES MACOSX_PACKAGE_LOCATION "Resources")
			target_sources(${_EXE_TARGET} PRIVATE ${ICON_PNG_PATH})
		elseif (INSTALL_DATA)
...
function(engine_install_deps TARGET)
	...
	foreach (DEP ${DEPS})
		...
		foreach (luasrc ${LUA_SRCS})
			...
			elseif (INSTALL_DATA)
				string(MAKE_C_IDENTIFIER "${INSTALL_DATA_DIR}/${luasrc}" _install_id)
		...
		endforeach()
		if (LUA_SRCS AND EMSCRIPTEN)
			target_link_options(${TARGET} PRIVATE "SHELL:--embed-file ${CMAKE_BINARY_DIR}/${PROJECT_NAME}/${datafiledir}@/${datafiledir}")
		endif()
	endforeach()
	...
	if (INSTALL_FILES)
		...
		foreach (datafile ${INSTALL_FILES})
			...
			string(MAKE_C_IDENTIFIER "${INSTALL_DATA_DIR}/${target_datafile}" _install_id)
			get_property(_already_installed GLOBAL PROPERTY _ENGINE_INSTALLED_${_install_id})
			if (NOT _already_installed)
				set_property(GLOBAL PROPERTY _ENGINE_INSTALLED_${_install_id} TRUE)
...
function(engine_emscripten_export_functions TARGET)
	...
	get_target_property(TMP_LINKER_FLAGS ${TARGET} LINK_FLAGS)
	set_target_properties(${TARGET} PROPERTIES LINK_FLAGS "${TMP_LINKER_FLAGS} -sEXPORTED_FUNCTIONS=[${FUNCTIONS_LIST}]")
endfunction()
