...
void setup() {
	...
	const core::VarPtr &glVersion = core::getVar(cfg::ClientOpenGLVersion);
...
#endif
	const core::VarPtr &multisampleBuffers = core::getVar(cfg::ClientMultiSampleBuffers);
	const core::VarPtr &multisampleSamples = core::getVar(cfg::ClientMultiSampleSamples);
...
static void handleVSync() {
	const bool vsync = core::getVar(cfg::ClientVSync)->boolVal();
	if (vsync) {
		if (!setVSync(-1)) {
			if (!setVSync(1)) {
...
bool init(int windowWidth, int windowHeight, float scaleFactor) {
	...
	if (contextFlags & SDL_GL_CONTEXT_DEBUG_FLAG) {
		const int severity = core::getVar(cfg::ClientDebugSeverity)->intVal();
