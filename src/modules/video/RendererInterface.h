...
/**
 * @brief Register descriptor bindings for a shader program (generated by shadertool).
 * On Vulkan this is used to build the descriptor set layout. On desktop GL this is
 * a no-op because layout(binding=N) already assigns the points. On GLES/WebGL the
 * binding qualifier is stripped, so this wires glUniformBlockBinding.
 */
